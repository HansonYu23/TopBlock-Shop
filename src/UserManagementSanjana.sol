...
contract UserManagementSanjana {
    ...
    function viewRole() public view returns (uint8) {
        ...
        if(roles[msg.sender] == 0) {
            return 0;
        }else if (roles[msg.sender] == 1){
            return 1;
        } else if (roles[msg.sender] == 2) {
            return 2;
    ...
    function sendInvoice(address toAddr, uint256 amount) public returns (bool) {
        ...
        // add your implementation here
        // ============================
        uint256 invoiceAddress = addr2invoice[msg.sender];
        if(roles[msg.sender] == 2 && roles[toAddr] == 3 && invoiceAddress == 0 && invoices[invoiceAddress].remainingAmount == 0) {
            //New Invoice 
    ...
    function viewInvoice() public view returns (uint256, uint256, address, address) {
        ...
        // add your implementation here
        // ============================
        if(roles[msg.sender] == 2 || roles[msg.sender] == 3) {
            uint256 invoiceAddress = addr2invoice[msg.sender];
            if(invoiceAddress == 0 || invoiceAddress >= invoices.length) {
                return (0, 0, address(0), address(0));
            }
            
            if((invoices[invoiceAddress].host == msg.sender && roles[msg.sender] == 2) || (roles[msg.sender] == 3 && invoices[invoiceAddress].guest == msg.sender)) {
                return (invoices[invoiceAddress].amount, invoices[invoiceAddress].remainingAmount, invoices[invoiceAddress].host, invoices[invoiceAddress].guest);
            }else {
                return (0, 0, address(0), address(0));
            }
    ...
    function payInvoice(uint256 amount) public returns (bool) {
        ...
        // add your implementation here
        // ============================
        if(roles[msg.sender] == 3 && amount <= balances[msg.sender]) {
            uint256 invoiceAddress = addr2invoice[msg.sender];
            uint256 payAmount = amount;
            if(amount > invoices[invoiceAddress].remainingAmount) {
                payAmount = invoices[invoiceAddress].remainingAmount;
            }
            balances[msg.sender] -= payAmount;
            balances[invoices[invoiceAddress].host] += payAmount;
